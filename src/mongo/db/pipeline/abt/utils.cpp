...
ABT generateCoerceToBool(ABT input, const std::string& varName) {
    ...
    ABT checkExists = make<FunctionCall>("exists", ABTVector{input});
    ABT checkNotNull =
        make<UnaryOp>(Operations::Not, make<FunctionCall>("isNull", ABTVector{input}));
...
void maybeComposePath(ABT& composition, ABT child) {
    if (child == make<PathIdentity>()) {
        return;
    }
    if (composition == make<PathIdentity>()) {
        composition = std::move(child);
        return;
    }

    composition = make<PathComposeM>(std::move(composition), std::move(child));
}
