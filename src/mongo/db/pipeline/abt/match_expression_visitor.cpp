...
public:
    ABTMatchExpressionVisitor(ABTExpressionContext& ctx, const bool allowAggExpressions)
    ...
    void visit(const ElemMatchValueMatchExpression* expr) override {
        // Returns true if at least one sub-object matches.
        ...
        ABT composition = _ctx.pop();
        for (size_t i = 1; i < childCount; i++) {
            maybeComposePath(composition, _ctx.pop());
        }

        uassert(6624000, "Empty field name for $elemMatch.", !expr->path().empty());

        ABT result = translateFieldPath(
    ...
    void visit(const ExprMatchExpression* expr) override {
        uassert(0, "Cannot generate an agg expression in this context", _allowAggExpressions);
