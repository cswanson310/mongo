...
ExchangeConsumer::ExchangeConsumer(std::unique_ptr<PlanStage> input,
    ...
    if (policy == ExchangePolicy::hashpartition || policy == ExchangePolicy::rangepartition) {
        uassert(0, "partition expression must be present", _state->partitionExpr());
    } else {
        uassert(0, "partition expression must not be present", !_state->partitionExpr());
    }
}
    ...
    while (_children[0]->getNext() == PlanState::ADVANCED) {
        ...
        switch (_state->policy()) {
            ...
            case ExchangePolicy::rangepartition: {
                ...
                uassert(4822841, "wrong partitioning", tag == sbe::value::TypeTags::NumberInt64);
                size_t idx = sbe::value::bitcastTo<size_t>(val);
                uassert(4822842, "wrong partitioning", idx < _pipes.size());
