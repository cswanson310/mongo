...
public:
    ...
    std::unique_ptr<sbe::EExpression> transport(const T&, Ts&&...) {
        uasserted(0, "abt tree is not lowered correctly");
        return nullptr;
    }
...
public:
    ...
    std::unique_ptr<sbe::PlanStage> walk(const T&, Ts&&...) {
        if constexpr (std::is_base_of_v<LogicalNode, T>) {
            uasserted(0, "A physical plan should not contain exclusively logical nodes.");
        }
        return nullptr;
    }
...
private:
    ...
    const properties::Properties& getPhysicalProperties(const Node& n) const;
