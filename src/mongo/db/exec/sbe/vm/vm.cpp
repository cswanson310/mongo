...
std::tuple<bool, value::TypeTags, value::Value> ByteCode::getFieldOrElement(
    ...
    if (value::isArray(objTag) && value::isString(fieldTag)) {
        int idx;
        auto status = NumberParser{}(value::getStringDataView(fieldTag, fieldValue), &idx);
        if (!status.isOK()) {
            return {false, value::TypeTags::Nothing, 0};
        }
        return getElement(
...
std::tuple<bool, value::TypeTags, value::Value> ByteCode::traverseP_nested(const CodeFragment* code,
                                                                           ...
                                                                           value::Value val) {
    if (value::isArray(tag)) {
        auto [tagArr, valArr] = value::makeNewArray();
        auto arr = value::getArrayView(valArr);
        value::ValueGuard guard{tag, valArr};

        for (value::ArrayEnumerator enumerator(tag, val); !enumerator.atEnd();
             enumerator.advance()) {
            ...
            arr->push_back(retTag, retVal);
        }

        guard.reset();
        return {true, tagArr, valArr};
    } else {
...
std::tuple<bool, value::TypeTags, value::Value> ByteCode::traverseF(const CodeFragment* code) {
    ...
    auto [own, tag, val] = getFromStack(2);
    ...
    if (value::isArray(tag)) {
        ...
        // If this is a filter over a number path then run over the whole array.
...
std::tuple<bool, value::TypeTags, value::Value> ByteCode::setField() {
    ...
    // TODO - implement a moveFromStack optimization.
    ...
    if (newTag == value::TypeTags::Nothing) {
        ...
        obj->push_back(fieldName, newTag, newVal);
...
std::tuple<bool, value::TypeTags, value::Value> ByteCode::builtinKeepFields(ArityType arity) {
    ...
    std::set<std::string, std::less<>> keepFieldsSet;
