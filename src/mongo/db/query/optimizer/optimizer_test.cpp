...
TEST(Optimizer, RefExplain) {
    ...
    ASSERT_EQ(
        ...
        "    [ptest]\n"
        "      Source []\n",
        ExplainGenerator::explain(scanNode));
...
TEST(Optimizer, CoScan) {
    ...
    ASSERT_EQ(
        ...
        "    skip: 0\n"
        "  CoScan []\n",
        ExplainGenerator::explain(limitNode));
}
...
TEST(Optimizer, Basic) {
    ...
    ABT evalNode = make<EvaluationNode>(
        ...
        make<EvalPath>(make<PathConstant>(Constant::int64(2)), make<Variable>("ptest")),
        std::move(filterNode));
    ABT joinNode = make<InnerMultiJoinNode>(makeSeq(std::move(evalNode)));
    ...
    ASSERT_EQ(
        ...
        "            [ptest]\n"
        "              Source []\n",
        ExplainGenerator::explain(clonedNode));
...
TEST(Optimizer, WindUnwind) {
    ...
    ABT windNode = make<WindNode>("p2", "p2pid", "p1", std::move(unwindNode));

    ABT rootWind =
        make<RootNode>(properties::ProjectionRequirement{ProjectionNameVector{"p1", "p2", "p2pid"}},
                       std::move(windNode));

    {
        auto env = VariableEnvironment::build(rootWind);
        ProjectionNameSet projSet = env.topLevelProjections();
        ProjectionNameSet expSet = {"p1", "p2", "p2pid"};
        ASSERT(expSet == projSet);
        ASSERT(!env.hasFreeVariables());
    ...
    ASSERT_EQ(
        ...
        "            [p1]\n"
        "              Source []\n",
        ExplainGenerator::explain(rootWind));
}
...
TEST(Optimizer, Collation) {
    ...
    ASSERT_EQ(
        ...
        "        [a]\n"
        "          Source []\n",
        ExplainGenerator::explain(collationNode));
}
...
TEST(Optimizer, LimitSkip) {
    ...
    ASSERT_EQ(
        ...
        "        [a]\n"
        "          Source []\n",
        ExplainGenerator::explain(limitSkipNode));
}
...
TEST(Optimizer, Distribution) {
    ...
    ASSERT_EQ(
        ...
        "        [a]\n"
        "          Source []\n",
        ExplainGenerator::explain(exchangeNode));
}
...
TEST(Properties, Basic) {
    ...
    ASSERT_TRUE(collation1.isCompatibleWith(collation2));
    ASSERT_FALSE(collation2.isCompatibleWith(collation1));
