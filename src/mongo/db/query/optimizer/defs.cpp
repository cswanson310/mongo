...
static std::unordered_map<ProjectionName, size_t> createMapFromVector(
    const ProjectionNameVector& v) {
    std::unordered_map<ProjectionName, size_t> result;
...
CostType CostType::operator-(const CostType& other) const {
    uassert(6624000, "Cannot subtract an infinite cost", other != kInfinity);
...
CostType CostType::fromDouble(const double cost) {
    uassert(6624000, "Cost is negative", cost >= 0.0);
