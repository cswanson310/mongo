...
inline void assertExprSort(const ABT& e) {
    if (!e.is<ExpressionSyntaxSort>()) {
        uasserted(6624010, "expression syntax sort expected");
    }
}

inline void assertPathSort(const ABT& e) {
    if (!e.is<PathSyntaxSort>()) {
        uasserted(6624011, "path syntax sort expected");
    }
}
 ...
 *
 * A canonical examples are: GROUP BY "a", ORDER BY "b", etc.
 *
 * We want to capture these references. The rule of ABTs says that the ONLY way to refecence a named
