...
SargableNode::SargableNode(PartialSchemaRequirements reqMap,
    ...
    assertNodeSort(getChild());
    uassert(0, "Empty requirements map", !_reqMap.empty());
}
...
HashJoinNode::HashJoinNode(JoinType joinType,
    ...
    uassert(0, "Invalid key sizes", !_leftKeys.empty() && _leftKeys.size() == _rightKeys.size());
    assertNodeSort(getLeftChild());
    assertNodeSort(getRightChild());
}
...
InnerMultiJoinNode::InnerMultiJoinNode(ABTVector children) : Base(std::move(children)) {
    for (const auto& child : nodes()) {
        assertNodeSort(child);
    }
}

bool InnerMultiJoinNode::operator==(const InnerMultiJoinNode& other) const {
...
UnionNode::UnionNode(ProjectionNameVector unionProjectionNames, ABTVector children)
    ...
    uassert(6624056, "Empty union", !unionProjectionNames.empty());
...
GroupByNode::GroupByNode(ProjectionNameVector groupByProjectionNames,
    ...
    : Base(std::move(child),
      ...
      _isLocal(isLocal),
      _canRewriteIntoLocal(canRewriteIntoLocal) {
    assertNodeSort(getChild());
}
...
bool GroupByNode::operator==(const GroupByNode& other) const {
    return getAggregationProjectionNames() == other.getAggregationProjectionNames() &&
        ...
        getGroupByProjectionNames() == other.getGroupByProjectionNames() &&
        _isLocal == other._isLocal && _canRewriteIntoLocal == other._canRewriteIntoLocal &&
        getChild() == other.getChild();
}
...
bool GroupByNode::isLocal() const {
    return _isLocal;
}

bool GroupByNode::canRewriteIntoLocal() const {
    return _canRewriteIntoLocal;
}
...
WindNode::WindNode(ProjectionName projectionName,
                   ProjectionName pidProjectionName,
                   ProjectionName cidProjectionName,
                   ABT child)
    : Base(std::move(child),
           buildSimpleBinder(ProjectionNameVector{projectionName}),
           make<References>(ProjectionNameVector{
               projectionName, std::move(pidProjectionName), std::move(cidProjectionName)})) {
    assertNodeSort(getChild());
}

bool WindNode::operator==(const WindNode& other) const {
    return binder() == other.binder() && getChild() == other.getChild();
}

const ABT& WindNode::getChild() const {
    return get<0>();
}

ABT& WindNode::getChild() {
    return get<0>();
}

CollationNode::CollationNode(properties::CollationRequirement property, ABT child)
    : Base(std::move(child),
           buildReferences(extractReferencedColumns(properties::makeProperties(property)))),
...
ExchangeNode::ExchangeNode(const properties::DistributionRequirement distribution,
                           const bool preserveSort,
                           ABT child)
    ...
    uassert(6624000,
            ...
            distribution.getType() != DistributionType::UnknownPartitioning);
}

bool ExchangeNode::operator==(const ExchangeNode& other) const {
    return _distribution == other._distribution && _preserveSort == other._preserveSort &&
        getChild() == other.getChild();
}
...
bool ExchangeNode::getPreserveSort() const {
