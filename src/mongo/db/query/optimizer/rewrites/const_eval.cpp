...
void ConstEval::transport(ABT& n, const Variable& var) {
    ...
    if (!def.definition.empty()) {
        ...
        if (auto constant = def.definition.cast<Constant>(); constant && !_inRefBlock) {
            ...
            // This is a indirection to another variable. So we can skip.
            swapAndUpdate(n, def.definition);
        } else if (_singleRef.erase(&var)) {
            // If this is the only reference to some expression then substitute the variable.
            swapAndUpdate(n, def.definition);
            ...
            // If we are in the ref block we do not want to inline even if there is only a sigle
    ...
    if (op.op() == Operations::Add) {
        ...
        if (lhsConst && rhsConst) {
            ...
            if (isNumber(lhsTag) && isNumber(rhsTag)) {
                // So this is the addition operation and both arguments are number constants, hence
                ...
                switch (resultType) {
                    ...
                    case TypeTags::NumberDecimal: {
                        resultValue =
                            constFoldNumberHelper<Decimal128>(lhsTag, lhsValue, rhsTag, rhsValue);
                        break;
                    }
