...
struct CollectedInfo {
    using VarRefsMap = stdx::unordered_map<std::string, stdx::unordered_map<const Variable*, bool>>;

    /**
     * All resolved variables so far
     */
    stdx::unordered_map<const Variable*, Definition> useMap;
     ...
     */
    stdx::unordered_map<std::string, std::vector<const Variable*>> freeVars;

    /**
     * Per relational node projections.
     */
    stdx::unordered_map<const Node*, DefinitionsMap> nodeDefs;

    /**
     * Support for tracking of the last local variable reference.
     */
    VarRefsMap varLastRefs;
    stdx::unordered_set<const Variable*> lastRefs;
    ...
    void merge(CollectedInfo&& other) {
        ...
        // copied.
        uassert(6624050, "use map is not empty", other.useMap.empty());

        defs.merge(other.defs);

        // Projection names are globally unique so everything should be copied.
        uassert(6624051, "duplicate projections", other.defs.empty());
        ...
        // copied.
        uassert(6624060, "duplicate nodes", other.nodeDefs.empty());

        // Merge last references.
        mergeLastRefs(std::move(other.varLastRefs));
        lastRefs.merge(other.lastRefs);
        uassert(6624061, "duplicate lastRefs", other.lastRefs.empty());
    }
    ...
    void mergeLastRefsImpl(VarRefsMap&& other, bool resetOther, bool resetBoth) {
        ...
        uassert(0, "varLastRefs must be empty", other.empty());
    }
    ...
    void assertEmptyDefs() {
        uassert(6624061, "Definitions must be empty", defs.empty());
    }
};
...
struct Collector {
    ...
    CollectedInfo transport(const ABT& n, const ScanNode& scanNode, CollectedInfo /*bindResult*/) {
        return collectForScan(n, scanNode, scanNode.binder(), {});
    }

    CollectedInfo transport(const ABT& n,
                            const PhysicalScanNode& physScanNode,
                            CollectedInfo /*bindResult*/) {
        return collectForScan(n, physScanNode, physScanNode.binder(), {});
    }
    ...
    CollectedInfo transport(const ABT& n,
        ...
        uassert(6624000, "Uninitialized memo", _memo);
    ...
    CollectedInfo transport(const ABT& n,
        ...
        // Note that refsResults follows the structure as built by buildUnionReferences.
        size_t counter = 0;
        for (auto& u : childResults) {
            // Manually copy and resolve references of specific child.
            for (const auto& name : names) {
                uassert(6624059, "Union projection does not exist", u.defs.count(name) != 0);
    ...
    CollectedInfo transport(const ABT& n,
        ...
        uassert(6624057, "Unwind projection does not exist", result.defs.count(name) != 0);
    ...
    CollectedInfo transport(const ABT& n,
        ...
        const auto& name = windNode.getProjectionName();
        uassert(6624058, "Wind projection does not exist", result.defs.count(name) != 0);

        // Redefine wind projection.
        result.defs[name] = Definition{n.ref(), windNode.getProjection().ref()};

        result.mergeNoDefs(std::move(bindResult));

        result.nodeDefs[&windNode] = result.defs;
...
const DefinitionsMap& VariableEnvironment::getDefinitions(const Node* node) const {
    ...
    uassert(6624062, "node does not exist", it != _info->nodeDefs.end());
...
const DefinitionsMap& VariableEnvironment::getDefinitions(ABT::reference_type node) const {
    uassert(6624000, "Invalid node type", node.is<Node>());
